// Static version for GitHub Pages (no backend API)
// Loads JSON files directly from /data/ directory
...
// Load available dates by checking for JSON files
async function loadAvailableDates() {
    // Generate list of dates to check (last 30 days)
    const dates = [];
...
async function loadTodayArticles() {
    if (availableDates.length > 0) {
        await loadArticles(availableDates[0]);
    } else {
        document.getElementById('loading').style.display = 'none';
        document.getElementById('error').textContent = 'No articles available yet. Please run the aggregation.';
...
async function loadArticles(dateStr) {
    ...
    try {
        ...
        loading.style.display = 'none';
        error.textContent = `No articles found for ${dateStr}`;
